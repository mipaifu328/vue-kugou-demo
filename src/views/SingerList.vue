<template>
  <div class="singer_list">
    <mt-cell v-for="item in list" :to="`/singer/info/${item.singerid}`" is-link :title="item.singername" :key="item.singerid">
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$http
        .get(`/proxy/singer/list/${singerID}?json=true`)
        .then(({ data }) => {
          Indicator.close();
